 ...
 * @param {string} representation - Optional
 */
Confluence.prototype.putContent = function(id, version, title, content, callback, minorEdit, representation){
    var page = {
        "id": id,
        "type": "page",
        "title": title,
...
Confluence.prototype.convertWikiMarkup = function(content, callback) {
    request
        .post(this.config.baseUrl + this.config.apiPath + "/contentbody/convert/storage")
        .auth(this.config.username, this.config.password)
        .type('json')
        .send({
            value: content,
